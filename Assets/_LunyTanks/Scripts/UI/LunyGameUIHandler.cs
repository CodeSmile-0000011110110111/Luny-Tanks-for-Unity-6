...
namespace CodeSmile.Luny.Tanks
{
	...
	public sealed class LunyGameUIHandler : LunyScript
	{
		public LunyGameManager m_GameManager;               // Reference to the GameManager in the scene
        ...
        public LunyStartMenuSlot[] m_PlayerSlots;           // The Slots in the Start Menu that display the available tanks and handle players selections
        ...
        private LunyPauseMenu m_PauseMenu;                  // Reference to the pause menu (if present in the scene)
        ...
        private void Start()
        {
            ...
            if (LunyMobileUIControl.Instance != null)
                LunyMobileUIControl.Instance.Hide();
            ...
            // Pause Menu
            m_PauseMenu = FindAnyObjectByType<LunyPauseMenu>(FindObjectsInactive.Include);
            if (m_PauseMenu != null)
            {
                m_PauseMenu.Init();
        ...
        void StartGame()
        {
            ...
            List<LunyGameManager.PlayerData> playerData = new List<LunyGameManager.PlayerData>();
            foreach (var slot in m_PlayerSlots)
            {
                if (!slot.IsOpen)
                {
                    playerData.Add(new LunyGameManager.PlayerData()
                    {
            ...
            if (LunyMobileUIControl.Instance != null)
                LunyMobileUIControl.Instance.Show();
